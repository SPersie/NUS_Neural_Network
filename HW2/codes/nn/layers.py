...
class BiRNN(Layer):
    ...
    def forward(self, input):
        ...
        fwd_rnn = self.forward_rnn.forward(input)
        reverse_fwc_rnn = self._reverse_temporal_data(input, mask)

        bwd_rnn = self.backward_rnn.forward(reverse_fwc_rnn)
        reverse_bwd_rnn = self._reverse_temporal_data(bwd_rnn, mask)

        output = np.concatenate((fwd_rnn, reverse_bwd_rnn), axis=2)
        #####################################################################################
